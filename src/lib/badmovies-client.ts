...
export class BadMoviesClient {
  ...
  public async getMovie(idOrSlug: string): Promise<NormalizedMovie | null> {
    ...
    try {
      const data = await this.fetch.request<Record<string, unknown>>(`${this.config.api.badMoviesBaseUrl}/movies/${idOrSlug}`);
      ...
      const movie: NormalizedMovie = {
        ...
        rating: (data.voteAverage as string | undefined) || (data.imdbRating as string | undefined) || 'N/A',
        source: 'badmovies' as const
      };
      this.cache.set(cacheKey, movie);
      return movie;
    } catch (error: unknown) {
      logger.error(`[BadMovies] getMovie failed for "${idOrSlug}": ${error instanceof Error ? error.message : String(error)}`);
      return null;
    }
  }
  ...
  public async getExperiments(): Promise<NormalizedExperiment[]> {
    ...
    try {
      const response = await this.fetch.request<Record<string, unknown>>(`${this.config.api.badMoviesBaseUrl}/experiments`);
      ...
      const experiments = (response.experiments as unknown[]).map((item) => this.mapExperiment(item));

      this.cache.set(cacheKey, experiments, 600); // 10 min cache
      return experiments;
    } catch (error: unknown) {
      logger.error(`[BadMovies] getExperiments failed: ${error instanceof Error ? error.message : String(error)}`);
      return [];
    }
  }
  ...
  public async getExperiment(slug: string): Promise<NormalizedExperiment | null> {
    ...
    try {
      const response = await this.fetch.request<Record<string, unknown>>(`${this.config.api.badMoviesBaseUrl}/experiments/${slug}`);
      ...
      logger.error(`[BadMovies] getExperiment failed for "${slug}": ${error instanceof Error ? error.message : String(error)}`);
      return null;
    }
  }
  ...
  private mapExperiment(item: unknown): NormalizedExperiment {
    ...
    const movies = Array.isArray(d.movies) 
      ? d.movies.map((m: unknown) => {
          ...
          const rawPoster = (mv.posterUrl ?? mv.poster_url ?? mv.posterPath ?? mv.poster_path) as string | undefined;
          const posterUrl = rawPoster?.startsWith('/') ? `https://badmovies.co${rawPoster}` : rawPoster;
          return { 
            id: mv.id?.toString() ?? mv.Id?.toString() ?? (mv.slug as string | undefined) ?? (mv.title as string | undefined) ?? '',
    ...
    if (movies.length === 0 && !Array.isArray(d.movies) && !d.movieTitles && !d.lineup) {
      logger.warn(`[BadMoviesClient] mapExperiment: no movies, movieTitles, or lineup found on item — name: ${String(d.title ?? d.name ?? '(unknown)')}`);
    }
    ...
    return {
      ...
      result: (d.result || `${String(d.movieCount ?? (Array.isArray(d.movies) ? d.movies.length : 0))} movies recorded`) as string,
  ...
  public async getStats(): Promise<NormalizedStats | null> {
    ...
    try {
      const response = await this.fetch.request<Record<string, unknown>>(`${this.config.api.badMoviesBaseUrl}/stats/summary`);
      ...
      if (experiments.length > 0) {
        const sorted = [...experiments].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
      ...
      logger.error(`[BadMovies] getStats failed: ${error instanceof Error ? error.message : String(error)}`);
      return null;
    }
  }

  public async globalSearch(query: string): Promise<{ movies: NormalizedMovie[], experiments: NormalizedExperiment[] }> {
    const cacheKey = this.cache.generateKey('badmovies:globalsearch', { query });
    const cached = this.cache.get<{ movies: NormalizedMovie[], experiments: NormalizedExperiment[] }>(cacheKey);
    if (cached) return cached;

    try {
      // API search for movies and experiments separately as no combined endpoint exists
      const [movieRes, expRes] = await Promise.all([
        this.fetch.request<Record<string, unknown>>(`${this.config.api.badMoviesBaseUrl}/movies?search=${encodeURIComponent(query)}`),
        this.fetch.request<Record<string, unknown>>(`${this.config.api.badMoviesBaseUrl}/experiments?search=${encodeURIComponent(query)}`)
      ]);
      ...
      logger.error(`[BadMovies] globalSearch failed for "${query}": ${error instanceof Error ? error.message : String(error)}`);
  ...
  public async getRandomMovie(): Promise<NormalizedMovie | null> {
    try {
      const response = await this.fetch.request<Record<string, unknown>>(`${this.config.api.badMoviesBaseUrl}/movies/random?limit=1`);
      if (!response || !Array.isArray(response.movies) || response.movies.length === 0) return null;
      return this.mapMovie(response.movies[0]);
    } catch (error: unknown) {
      logger.error(`[BadMovies] getRandomMovie failed: ${error instanceof Error ? error.message : String(error)}`);
      return null;
    }
  }

  public async getRandomQuote(): Promise<{ text: string, movie: string, year: string, movieSlug?: string } | null> {
    try {
      const response = await this.fetch.request<Record<string, unknown>>(`${this.config.api.badMoviesBaseUrl}/quotes/random`);
      ...
      logger.error(`[BadMovies] getRandomQuote failed: ${error instanceof Error ? error.message : String(error)}`);
      return null;
    }
  }
  ...
  private mapMovie(item: unknown): NormalizedMovie {
    ...
    return {
      ...
      year: typeof d.releaseDate === 'string' ? d.releaseDate.split('-')[0] : ((d.year || 'Unknown') as string).toString(),
      description: (d.overview as string) || '',
      url: `https://badmovies.co/movies/${d.slug as string}`,
      posterUrl,
      rating: (d.voteAverage as string | undefined) || (d.imdbRating as string | undefined) || 'N/A',
      source: 'badmovies' as const
    };
  }
}
