<script lang="ts">
	...
	const navGroups = [
		{
			label: 'Overview',
			icon: `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-bar-chart-3"><path d="M3 3v18h18"/><path d="M18 17V9"/><path d="M13 17V5"/><path d="M8 17v-3"/></svg>`,
			items: [
				{
					name: 'Dashboard',
					path: '/',
		...
		{
			label: 'Content',
			icon: `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-database"><ellipse cx="12" cy="5" rx="9" ry="3"/><path d="M3 5V19A9 3 0 0 0 21 19V5"/><path d="M3 12A9 3 0 0 0 21 12"/></svg>`,
			items: [
				{
					name: 'Movies',
					path: '/movies',
		...
		{
			label: 'System',
			icon: `<svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="lucide lucide-shield-check"><path d="M20 13c0 5-3.5 7.5-7.66 8.95a1 1 0 0 1-.67-.01C7.5 20.5 4 18 4 13V6a1 1 0 0 1 1-1c2 0 4.5-1.2 6.24-2.72a1.06 1.06 0 0 1 1.52 0C14.51 3.81 17 5 19 5a1 1 0 0 1 1 1z"/><path d="m9 12 2 2 4-4"/></svg>`,
			items: [
				{
					name: 'Users',
					path: '/users',
	...
	<div class="flex-1 overflow-y-auto py-4">
		<nav class="space-y-0.5 px-3">
			{#each navGroups as group, groupIndex (groupIndex)}
				{#if collapsed}
					{#if groupIndex > 0}
						<div class="mx-2 h-px bg-slate-700/50"></div>
					{/if}
				{:else}
					<div class="flex items-center gap-2 px-4 pt-3 pb-1.5 {groupIndex === 0 ? 'pt-0' : ''}">
						<span class="shrink-0 text-cyan-400">
							<!-- eslint-disable-next-line svelte/no-at-html-tags -->
							{@html group.icon}
						</span>
						<span
							class="text-label font-bold tracking-widest text-cyan-400/80 uppercase drop-shadow-glow-cyan-sm"
							>{group.label}</span
						>
					</div>
				{/if}
