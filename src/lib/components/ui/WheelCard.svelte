<script lang="ts">
	...
	function flipIn(
		_node: HTMLElement,
		{ duration = 200, delay = 200 }: { duration?: number; delay?: number } = {}
	) {
		return {
			duration,
			delay,
...
<div class="relative">
	{#if inEditMode}
		...
		>
			...
			<div class="flex flex-1 flex-col gap-3 p-4">
											...
											>
												...
												<div class="min-w-0 flex-1">
													<div class="flex items-baseline gap-2">
														...
														>
														{#if result.year}
															<span class="shrink-0 font-mono text-ui text-slate-500"
																>{result.year}</span
															>
														{/if}
													</div>
													{#if result.overview}
														<p class="mt-0.5 line-clamp-2 text-ui text-slate-500">
															{result.overview}
														</p>
													{/if}
												</div>
				...
				{#if movieWatched}
					<div
						class="flex items-start gap-2 rounded-none border border-amber-500/40 bg-amber-500/10 px-3 py-2.5"
					>
						<svg
							xmlns="http://www.w3.org/2000/svg"
							width="14"
	...
	{:else if entry}
		...
		>
			...
			<div
				class="absolute top-2 right-2 z-20 flex gap-2 opacity-0 transition-opacity duration-200 group-hover:opacity-100"
			>
				<button
					onclick={onEdit}
					class="rounded-none border border-slate-700/80 bg-slate-950/70 px-2 py-1 text-label font-bold tracking-wider text-slate-300 uppercase backdrop-blur-sm transition-colors hover:border-cyan-500/60 hover:text-cyan-400"
				>
					Edit
				</button>
				<button
					onclick={onDelete}
					class="rounded-none border border-slate-700/80 bg-slate-950/70 px-2 py-1 text-label font-bold tracking-wider text-slate-500 uppercase backdrop-blur-sm transition-colors hover:border-rose-500/60 hover:text-rose-500"
				>
					Delete
				</button>
			</div>
			...
			<div class="absolute inset-x-0 bottom-0 z-20 flex flex-col gap-1.5 p-4">
				...
				<div class="flex items-end justify-between gap-2">
					<h3
						class="text-base leading-snug font-semibold text-slate-100 drop-shadow-sm group-hover:text-cyan-300"
					>
						{entry.title}
					</h3>
					{#if entry.year}
