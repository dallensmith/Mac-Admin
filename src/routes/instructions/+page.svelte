	...
	<div class="flex items-start gap-3">
		...
		<div>
			...
			<ul class="mt-1 list-inside list-disc text-xs text-rose-300/80">
				...
				<li>
					Test profiles using the <code class="font-mono">experimental</code> tag before setting them
					active.
				</li>
	...
	<div class="space-y-6 xl:col-span-6">
		...
		{#if selectedTemplate}
					...
					<div class="flex items-start gap-3">
						...
						<div>
							...
							<p class="mt-1 text-xs text-slate-500">
								This profile is populated from the bot's <code
									class="rounded bg-slate-700/60 px-1 py-0.5 font-mono text-slate-400"
									>config/instructions/</code
								> files on startup. Changes must be made there and reseeded.
							</p>
						</div>
					</div>
				</div>
			...
			<form id="updateForm" method="POST" action="?/update" use:enhance={enhanceUpdate}>
				...
				<SectionCard title="Profile">
					<div class="grid gap-4 sm:grid-cols-2">
						<div>
							...
							<input
								...
								class="input-dark {isReadonly ? 'cursor-default opacity-60' : ''} {updateNameError
						...
						<div class="sm:col-span-2">
							...
							<textarea
								...
								readonly={isReadonly}
								class="input-dark {isReadonly ? 'cursor-default opacity-60' : ''}"
							></textarea>
						</div>
					</div>
					<div class="mt-6 flex flex-wrap gap-3 border-t border-slate-800/50 pt-4">
						...
						<button
							...
							form="deleteForm"
							disabled={saving ||
								(selectedTemplate.is_active as boolean) ||
								(selectedTemplate.is_default as boolean) ||
				...
				<SectionCard title="Personality">
					<p class="mb-4 text-xs text-slate-500">
						Define the bot's persona, conduct rules, and resource knowledge. Each tab overrides the
						bot's filesystem default — leave empty to use the file-based fallback.
					</p>
					<div class="mb-4 flex flex-wrap gap-1 border-b border-slate-800/80 pb-px">
						{#each [{ key: 'system' as const, label: 'Persona' }, { key: 'behavior' as const, label: 'Conduct' }, { key: 'resources' as const, label: 'Resources' }] as tab (tab.key)}
							<button
								type="button"
								class="border-b-2 px-3 py-2 text-ui font-bold tracking-widest uppercase transition-all {personalityTab ===
								tab.key
									? 'border-cyan-500 text-cyan-400'
									: 'border-transparent text-slate-500 hover:text-slate-300'}"
								onclick={() => (personalityTab = tab.key)}
							>
								{tab.label}
							</button>
						{/each}
					...
					<textarea
						name="system"
						bind:value={edits.system}
						readonly={isReadonly}
						class="h-100 w-full resize-none rounded border border-slate-800/80 bg-slate-950/50 p-4 font-mono text-sm text-slate-300 shadow-inset-form transition-all duration-300 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/50 focus:outline-none {isReadonly
							? 'cursor-default opacity-70 focus:border-slate-800/80 focus:ring-0'
							: ''} {personalityTab !== 'system' ? 'hidden' : ''}"
					></textarea>
					<textarea
						name="behavior"
						bind:value={edits.behavior}
						readonly={isReadonly}
						class="h-100 w-full resize-none rounded border border-slate-800/80 bg-slate-950/50 p-4 font-mono text-sm text-slate-300 shadow-inset-form transition-all duration-300 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/50 focus:outline-none {isReadonly
							? 'cursor-default opacity-70 focus:border-slate-800/80 focus:ring-0'
							: ''} {personalityTab !== 'behavior' ? 'hidden' : ''}"
					></textarea>
					<textarea
						name="resources"
						bind:value={edits.resources}
						readonly={isReadonly}
						class="h-100 w-full resize-none rounded border border-slate-800/80 bg-slate-950/50 p-4 font-mono text-sm text-slate-300 shadow-inset-form transition-all duration-300 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500/50 focus:outline-none {isReadonly
							? 'cursor-default opacity-70 focus:border-slate-800/80 focus:ring-0'
							: ''} {personalityTab !== 'resources' ? 'hidden' : ''}"
					></textarea>
				</SectionCard>
				...
				<SectionCard title="Conversation Settings">
					<p class="mb-5 text-xs text-slate-500">
						Configure how the bot tracks conversation state, detects its name, and recognizes
						termination signals.
					</p>
					...
					<div class="mb-6">
						...
						<p class="mt-0.5 mb-3 text-xs text-slate-500">
							Phrases that signal the user is addressing the bot by name.
						</p>
						<div class="flex flex-wrap items-center gap-2">
							{#each convNameTriggers as trigger, i (i)}
								<span
									class="flex items-center gap-1 rounded border border-slate-700/60 bg-slate-900/50 px-2.5 py-1"
								>
									<span class="font-mono text-xs text-slate-300">{trigger}</span>
									{#if !isReadonly}
										<button
											type="button"
											onclick={() => removeConvNameTrigger(i)}
											class="ml-1 text-slate-500 transition-colors hover:text-rose-400"
											aria-label="Remove"
										>
											<svg
												xmlns="http://www.w3.org/2000/svg"
							...
							{#if !isReadonly}
								<input
									...
									class="min-w-32 flex-1 rounded border border-dashed border-slate-700 bg-transparent px-2.5 py-1 font-mono text-xs text-slate-300 transition-colors outline-none placeholder:text-slate-600 hover:border-slate-600 focus:border-cyan-500/60"
									onkeydown={(e) => {
										if (e.key === 'Enter') {
											e.preventDefault();
											addConvNameTrigger(convNameInput);
											convNameInput = '';
										}
									}}
								/>
						...
						{#if convNameRegex}
							<div class="mt-3">
								...
								<div
									class="rounded border border-slate-800/60 bg-slate-950/50 px-3 py-2 font-mono text-xs text-slate-500 select-all"
								>
									{convNameRegex}
								</div>
							</div>
							...
							{#each convTermTriggers as trigger, i (i)}
								...
								>
									...
									{#if !isReadonly}
										<button
											type="button"
											onclick={() => removeConvTermTrigger(i)}
											class="ml-1 text-slate-500 transition-colors hover:text-rose-400"
											aria-label="Remove"
										>
											<svg
							...
							{#if !isReadonly}
								<input
									...
									onkeydown={(e) => {
										if (e.key === 'Enter') {
											e.preventDefault();
											addConvTermTrigger(convTermInput);
											convTermInput = '';
										}
									}}
								/>
				...
				<SectionCard title="Response Templates">
					<p class="mb-4 text-xs text-slate-500">
						Select which Discord embed templates are active for this profile. Leave all unchecked to
						enable every template.
					</p>
					...
					{#if (data.discordTemplates as RecordModel[]).length === 0}
						<p class="text-xs text-slate-500 italic">
							No Discord templates found. <a
								href="/discord-templates"
								class="text-cyan-400 underline hover:text-cyan-300">Create one →</a
							>
						</p>
					{:else}
						<div class="space-y-2">
							{#each data.discordTemplates as dt (dt.id)}
								...
								<label
									class="flex cursor-pointer items-center gap-3 rounded border border-slate-800/60 bg-slate-900/40 px-3 py-2.5 transition-colors hover:border-slate-700 {isReadonly
										? 'cursor-default'
										: ''}"
								>
									<input
				...
				<SectionCard title="Output Discipline">
					<p class="mb-5 text-xs text-slate-500">
						Controls how the bot formats its responses. Leave any field empty to use the bot's
						built-in default for that section.
					</p>

					<div class="mb-2 flex items-center justify-between">
						<p class="text-xs text-slate-500">
							Full override replaces the entire OUTPUT DISCIPLINE block; granular fields are ignored
							when a full override is set.
						</p>
						<label
							class="flex items-center gap-2 rounded border border-slate-700/60 bg-slate-900/40 px-3 py-1.5 {isReadonly
								? 'cursor-default'
								: 'cursor-pointer'}"
						>
							<input
								type="checkbox"
								bind:checked={useOdOverride}
								disabled={isReadonly}
								class="h-3.5 w-3.5 rounded border-slate-600 bg-slate-800 accent-cyan-500"
							/>
					...
					{#if useOdOverride}
						<div class="rounded border border-amber-500/20 bg-amber-500/5 p-3">
							<p class="mb-2 text-xs text-amber-400/70">
								Replaces the entire OUTPUT DISCIPLINE block. All granular fields are ignored when
								this is non-empty.
							</p>
							<textarea
								...
								readonly={isReadonly}
								class="input-dark h-auto w-full resize-y font-mono text-sm {isReadonly
									? 'cursor-default opacity-70'
									: ''}"
								placeholder="Full OUTPUT DISCIPLINE block text…"
							></textarea>
						</div>
						...
						<div class="mt-4 space-y-5">
							...
							<div>
								...
								<p class="mb-1.5 text-xs text-slate-500">
									The "response is sent VERBATIM" paragraph. Empty = use seeded default.
								</p>
								<textarea
									name="od_verbatim_rule"
									rows="3"
									bind:value={edits.od_verbatim_rule}
									readonly={isReadonly}
									class="input-dark h-auto w-full resize-y font-mono text-sm {isReadonly
										? 'cursor-default opacity-70'
										: ''}"
									placeholder="Your response is sent VERBATIM to Discord…"
								></textarea>
							</div>
							<!-- od_banned_starters -->
							<div>
								...
								<p class="mb-2 text-xs text-slate-500">
									Response opener phrases the bot must never begin with. Empty list = use seeded
									default.
								</p>
								<input type="hidden" name="od_banned_starters" value={odBannedStartersJson} />
								<div class="space-y-2">
									{#each odBannedStarters as _starter, i (i)}
										<div class="flex items-center gap-2">
											<input
												...
												bind:value={odBannedStarters[i]}
												readonly={isReadonly}
												class="input-dark flex-1 font-mono text-sm {isReadonly
													? 'cursor-default opacity-70'
													: ''}"
											/>
											{#if !isReadonly}
								...
								{#if !isReadonly}
									<button
										type="button"
										onclick={addBannedStarter}
										class="mt-2 text-label font-bold tracking-widest text-cyan-400 uppercase transition-colors hover:text-cyan-300"
										>+ Add Starter</button
									>
								{/if}
							</div>
							...
							<div>
								...
								<p class="mb-2 text-xs text-slate-500">
									Wrong/right illustrative pairs shown to the bot. Empty = use seeded default.
								</p>
								<input type="hidden" name="od_examples" value={odExamplesJson} />
								<div class="space-y-3">
									{#each odExamples as example, i (i)}
										<div class="rounded border border-slate-800/60 bg-slate-900/40 p-3">
											<div class="mb-2 flex items-center justify-between">
												<span
													class="text-label-xs font-bold tracking-widest text-slate-500 uppercase"
													>Pair {i + 1}</span
												>
												{#if !isReadonly}
													<button
														type="button"
														onclick={() => removeOdExample(i)}
											...
											<div class="grid gap-2 sm:grid-cols-2">
												<div>
													...
													<textarea
														rows="2"
														placeholder="Bad response example…"
														bind:value={example.wrong}
														readonly={isReadonly}
														class="input-dark w-full resize-y text-sm {isReadonly
															? 'cursor-default opacity-70'
															: ''}"
													></textarea>
												</div>
												<div>
													...
													<textarea
														rows="2"
														placeholder="Good response example…"
														bind:value={example.right}
														readonly={isReadonly}
														class="input-dark w-full resize-y text-sm {isReadonly
															? 'cursor-default opacity-70'
															: ''}"
								...
								{#if !isReadonly}
									<button
										type="button"
										onclick={addOdExample}
										class="mt-2 text-label font-bold tracking-widest text-cyan-400 uppercase transition-colors hover:text-cyan-300"
										>+ Add Example Pair</button
									>
								{/if}
							</div>
							<!-- od_extra_rules -->
							<div>
								...
								<p class="mb-2 text-xs text-slate-500">
									Additional rules appended after the examples. Empty = none appended.
								</p>
								<input type="hidden" name="od_extra_rules" value={odExtraRulesJson} />
								<div class="space-y-3">
									{#each odExtraRules as rule, i (i)}
										<div class="rounded border border-slate-800/60 bg-slate-900/40 p-3">
											<div class="mb-2 flex items-center gap-2">
												<input
													type="text"
													placeholder="Label (e.g. No asterisks)"
													bind:value={rule.label}
													readonly={isReadonly}
													class="input-dark flex-1 text-sm {isReadonly
														? 'cursor-default opacity-70'
														: ''}"
												/>
												{#if !isReadonly}
				...
				<SectionCard title="Rules & Triggers">
					<p class="mb-5 text-xs text-slate-500">
						Layered rules and trigger phrases that modify bot behavior. Prompt rules are serialized
						into <code class="rounded bg-slate-700/60 px-1 py-0.5 font-mono text-slate-400"
							>conversation_rules</code
						>; custom rules and trigger phrases are stored separately.
					</p>
					...
					<div class="mb-6">
						...
						<p class="mb-3 text-xs text-slate-500">
							Injected before OUTPUT DISCIPLINE. Stored in <code
								class="rounded bg-slate-700/60 px-1 py-0.5 font-mono text-slate-400"
								>conversation_rules.promptRules</code
							>.
						</p>
						<div class="space-y-3">
								...
								<div class="rounded border border-slate-800/60 bg-slate-900/40 p-3">
									<div class="mb-2 flex items-center gap-2">
										...
										{#if !isReadonly}
											<button
												type="button"
												onclick={() => removeConvPromptRule(i)}
												class="shrink-0 rounded p-1.5 text-slate-500 transition-colors hover:bg-slate-800 hover:text-rose-400"
												aria-label="Remove rule"
											>
												<svg
								...
								<div class="rounded border border-slate-800/60 bg-slate-900/40 p-3">
									<div class="mb-2 flex items-start justify-between gap-2">
										<div class="grid flex-1 gap-2 sm:grid-cols-2">
											<input
												...
												readonly={isReadonly}
												class="input-dark text-sm {isReadonly ? 'cursor-default opacity-70' : ''}"
											/>
											<input
												type="text"
									...
									<div class="mb-2">
										...
										<textarea
											...
											readonly={isReadonly}
											class="input-dark w-full resize-y font-mono text-sm {isReadonly
												? 'cursor-default opacity-70'
												: ''}"
										></textarea>
									</div>
									<input
										...
										bind:value={tp.notes}
										readonly={isReadonly}
										class="input-dark w-full text-sm {isReadonly
											? 'cursor-default opacity-70'
											: ''}"
									/>
								</div>
			...
			<SectionCard title="Prompt Sections">
				<p class="mb-4 text-xs text-slate-500">
					Ordered prompt section records that fully replace the hardcoded TypeScript builders when
					present. Zero sections = fall back to hardcoded behavior.
				</p>
				...
				{#if templateSections.length === 0}
					<div class="mb-4 rounded border border-slate-800/50 bg-slate-900/30 p-6 text-center">
						...
						{#if !isReadonly}
							<form
								method="POST"
								action="?/duplicateSectionsFromDefault"
								use:enhance={enhanceSection}
								class="mt-3 inline-block"
							>
								<input type="hidden" name="instruction_set_id" value={selectedTemplate.id} />
								<button
									type="submit"
									class="text-label font-bold tracking-widest text-fuchsia-400 uppercase transition-colors hover:text-fuchsia-300"
									>Duplicate sections from Default →</button
								>
							</form>
						{/if}
					</div>
				{:else}
					<div class="space-y-3">
						{#each templateSections as section (section.id)}
							...
							<div
								class="rounded border {(edit?.enabled ?? section.enabled)
									? 'border-slate-700/60 bg-slate-900/40'
									: 'border-slate-800/40 bg-slate-900/20 opacity-60'} overflow-hidden"
							>
								<!-- Header row -->
								<div class="flex items-center gap-3 px-3 py-2.5">
									{#if !isReadonly}
										...
										>
											...
											<input
												type="hidden"
												name="section_id"
												value={edit?.section_id ?? section.section_id}
											/>
											<input type="hidden" name="label" value={edit?.label ?? section.label} />
											<input
												type="hidden"
												name="content"
												value={edit?.content ?? section.content}
											/>
											<input type="hidden" name="order" value={edit?.order ?? section.order} />
											<input
												type="hidden"
												name="enabled"
												value={String(!(edit?.enabled ?? section.enabled))}
											/>
											<input
												type="hidden"
												name="condition"
												value={edit?.condition ?? section.condition}
											/>
											<button
												type="submit"
												...
												onclick={() => {
													if (sectionEdits[section.id])
														sectionEdits[section.id].enabled = !sectionEdits[section.id].enabled;
												}}
											>
									...
									<div class="min-w-0 flex-1">
										<div class="flex items-center gap-2">
											<span class="truncate font-mono text-xs font-bold text-slate-300"
												>{edit?.label ?? section.label}</span
											>
											<span
												class="shrink-0 rounded px-1.5 py-0.5 text-label-xs font-bold tracking-widest uppercase {(edit?.condition ??
													section.condition) === 'always'
													? 'bg-cyan-500/10 text-cyan-400'
													: (edit?.condition ?? section.condition) === 'session_active'
										...
										<span class="font-mono text-xs text-slate-500"
											>{edit?.section_id ?? section.section_id}</span
										>
									</div>

									<span class="shrink-0 font-mono text-xs text-slate-600"
										>#{edit?.order ?? section.order}</span
									>
										...
										<form method="POST" action="?/updateSection" use:enhance={enhanceSection}>
											...
											<div class="mb-3 grid gap-3 sm:grid-cols-3">
												<div>
													...
													<input
														type="text"
														name="section_id"
														bind:value={edit.section_id}
														readonly={isReadonly}
														class="input-dark font-mono text-sm {isReadonly
															? 'cursor-default opacity-60'
															: ''}"
														placeholder="e.g. first_person"
													/>
												</div>
												<div>
													...
													<input
														...
														readonly={isReadonly}
														class="input-dark text-sm {isReadonly
															? 'cursor-default opacity-60'
															: ''}"
											...
											<div class="mb-3 flex items-center gap-3">
												...
												<input
													type="number"
													name="order"
													bind:value={edit.order}
													readonly={isReadonly}
													class="input-dark w-24 text-sm {isReadonly
														? 'cursor-default opacity-60'
														: ''}"
												/>
											</div>
											...
											{#if !isReadonly}
												<div class="flex items-center justify-between">
													<button
														type="submit"
														class="rounded border border-cyan-500/50 bg-cyan-500/10 px-3 py-1.5 text-label font-bold tracking-widest text-cyan-400 uppercase transition-colors hover:bg-cyan-500/20"
														>Save Section</button
													>
													<button
														type="submit"
														form="deleteSectionForm"
														onclick={() => (deleteSectionId = section.id)}
														class="rounded border border-rose-500/30 bg-rose-500/10 px-3 py-1.5 text-label font-bold tracking-widest text-rose-400 uppercase transition-colors hover:bg-rose-500/20"
														>Delete</button
													>
												</div>
											{/if}
				...
				{#if !isReadonly}
					<form
						...
						use:enhance={enhanceSection}
						class="mt-4 border-t border-slate-800/50 pt-4"
					>
						<input type="hidden" name="instruction_set_id" value={selectedTemplate.id} />
						...
						<input
							type="hidden"
							name="order"
							value={templateSections.length > 0
								? Math.max(...templateSections.map((s) => s.order as number)) + 10
								: 10}
						/>
	...
	<div class="space-y-6 xl:col-span-3">
		{#if selectedTemplate}
			<SectionCard title="Profile Summary">
				...
				<div class="mb-4 border-t border-slate-800/50 pt-4">
					...
					<div class="space-y-0.5 text-xs text-slate-500">
						<div>
							{convNameTriggers.length} name trigger{convNameTriggers.length !== 1 ? 's' : ''}
						</div>
						<div>
							{convTermTriggers.length} term signal{convTermTriggers.length !== 1 ? 's' : ''}
						</div>
					...
					{#if enabledTemplateKeys.length > 0}
						<div class="flex flex-wrap gap-1">
							{#each enabledTemplateKeys as k (k)}
								<span class="rounded bg-slate-800/80 px-1.5 py-0.5 font-mono text-xs text-slate-400"
									>{k}</span
								>
							{/each}
						</div>
					{:else}
						<p class="text-xs text-slate-500 italic">All enabled</p>
					{/if}
				</div>
				...
				<div class="mb-4 border-t border-slate-800/50 pt-4">
					...
					{#if useOdOverride}
						...
						<p class="mt-0.5 line-clamp-2 font-mono text-xs text-slate-400">
							{edits.output_discipline.slice(0, 80)}{edits.output_discipline.length > 80 ? '…' : ''}
						</p>
					{:else}
						<p class="text-xs text-slate-400">Granular fields</p>
						<div class="mt-1 space-y-0.5 text-xs text-slate-500">
							<div>
								{odBannedStarters.length} banned starter{odBannedStarters.length !== 1 ? 's' : ''}
							</div>
				...
				<div class="border-t border-slate-800/50 pt-4">
					...
					<div class="space-y-0.5 text-xs text-slate-500">
						<div>
							{templateSections.length} prompt section{templateSections.length !== 1 ? 's' : ''}
						</div>
						<div>
							{triggerPhrases.length} trigger phrase{triggerPhrases.length !== 1 ? 's' : ''}
						</div>
			...
			<SectionCard title="Assembled Prompt">
				...
				<pre
					class="max-h-96 overflow-y-auto rounded border border-slate-800/60 bg-slate-950/80 p-3 font-mono text-xs leading-relaxed whitespace-pre-wrap text-slate-300">{assembledPrompt}</pre>
			</SectionCard>
		{/if}
	</div>
</div>
