import { pgTable, index, foreignKey, serial, integer, varchar, numeric, text, timestamp, json, boolean, bigint, uniqueIndex } from "drizzle-orm/pg-core"
...
export const movieActors = pgTable("movie_actors", {
	...
	index("idx_movie_actors_optimized").using("btree", table.movieId.asc().nullsLast().op("int4_ops"), table.order.asc().nullsLast().op("int4_ops")).where(sql`("order" IS NOT NULL)`),
	index("movie_actors_movie_id_idx").using("btree", table.movieId.asc().nullsLast().op("int4_ops")),
	index("movie_actors_person_id_idx").using("btree", table.personId.asc().nullsLast().op("int4_ops")),
	foreignKey({
...
export const reviews = pgTable("reviews", {
	...
	index("reviews_movie_status_idx").using("btree", table.movieId.asc().nullsLast().op("int4_ops"), table.status.asc().nullsLast().op("int4_ops")),
	index("reviews_status_idx").using("btree", table.status.asc().nullsLast().op("text_ops")),
	index("reviews_user_movie_idx").using("btree", table.userId.asc().nullsLast().op("int4_ops"), table.movieId.asc().nullsLast().op("int4_ops")),
	foreignKey({
...
export const experimentMovies = pgTable("experiment_movies", {
	id: serial().notNull(),
	experimentId: integer("experiment_id").notNull(),
	movieId: integer("movie_id").notNull(),
	order: integer().default(0).notNull(),
}, (table) => [
	index("idx_experiment_movies_optimized").using("btree", table.experimentId.asc().nullsLast().op("int4_ops"), table.movieId.asc().nullsLast().op("int4_ops"), table.order.asc().nullsLast().op("int4_ops")),
	foreignKey({
...
export const listItems = pgTable("list_items", {
	...
	index("idx_list_items_list_order").using("btree", table.listId.asc().nullsLast().op("int4_ops"), table.orderPosition.asc().nullsLast().op("int4_ops")),
	index("idx_list_items_movie_id").using("btree", table.movieId.asc().nullsLast().op("int4_ops")),
	index("idx_list_items_order").using("btree", table.listId.asc().nullsLast().op("int4_ops"), table.orderPosition.asc().nullsLast().op("int4_ops")),
	foreignKey({
...
export const movieDirectors = pgTable("movie_directors", {
	...
	index("movie_directors_movie_id_idx").using("btree", table.movieId.asc().nullsLast().op("int4_ops")),
	index("movie_directors_person_id_idx").using("btree", table.personId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.movieId],
			foreignColumns: [movies.id],
...
export const listFollows = pgTable("list_follows", {
	...
	index("idx_list_follows_follower").using("btree", table.followerId.asc().nullsLast().op("int4_ops")),
...
export const games = pgTable("games", {
	...
	index("games_movie_id_created_at_idx").using("btree", table.movieId.asc().nullsLast().op("int4_ops"), table.createdAt.desc().nullsFirst().op("int4_ops")),
	foreignKey({
			columns: [table.movieId],
			foreignColumns: [movies.id],
...
export const movies = pgTable("movies", {
	...
	index("idx_movies_release_date_title").using("btree", table.releaseDate.desc().nullsFirst().op("text_ops"), table.title.asc().nullsLast().op("text_ops")).where(sql`(release_date IS NOT NULL)`),
	foreignKey({
...
export const movieStudios = pgTable("movie_studios", {
	...
	index("movie_studios_movie_id_idx").using("btree", table.movieId.asc().nullsLast().op("int4_ops")),
	index("movie_studios_studio_id_idx").using("btree", table.studioId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.movieId],
			foreignColumns: [movies.id],
...
export const movieWriters = pgTable("movie_writers", {
	...
	index("movie_writers_movie_id_idx").using("btree", table.movieId.asc().nullsLast().op("int4_ops")),
	index("movie_writers_person_id_idx").using("btree", table.personId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.movieId],
			foreignColumns: [movies.id],
...
export const trivia = pgTable("trivia", {
	...
	index("trivia_movie_id_created_at_idx").using("btree", table.movieId.asc().nullsLast().op("int4_ops"), table.createdAt.desc().nullsFirst().op("int4_ops")),
	foreignKey({
			columns: [table.movieId],
			foreignColumns: [movies.id],
...
export const movieStreamingSources = pgTable("movie_streaming_sources", {
	id: serial().notNull(),
	movieId: integer("movie_id").notNull(),
	...
	uniqueIndex("unique_movie_source_region_format").using("btree", sql`movie_id`, sql`source_id`, sql`region`, sql`COALESCE(format, 'none'::character varying)`),
	foreignKey({
			columns: [table.movieId],
			foreignColumns: [movies.id],
...
export const movieGenres = pgTable("movie_genres", {
	...
	index("idx_movie_genres_optimized").using("btree", table.movieId.asc().nullsLast().op("int4_ops"), table.genreId.asc().nullsLast().op("int4_ops")),
	index("movie_genres_genre_id_idx").using("btree", table.genreId.asc().nullsLast().op("int4_ops")),
	foreignKey({
			columns: [table.movieId],
			foreignColumns: [movies.id],
...
export const movieLanguages = pgTable("movie_languages", {
	id: serial().notNull(),
	movieId: integer("movie_id").notNull(),
...
export const userLists = pgTable("user_lists", {
	...
	index("idx_user_lists_created_at").using("btree", table.createdAt.desc().nullsFirst().op("timestamp_ops")),
	index("idx_user_lists_favorite").using("btree", table.userId.asc().nullsLast().op("bool_ops"), table.isFavoriteList.asc().nullsLast().op("int4_ops")).where(sql`(is_favorite_list = true)`),
	index("idx_user_lists_is_public").using("btree", table.isPublic.asc().nullsLast().op("bool_ops")),
	index("idx_user_lists_public").using("btree", table.isPublic.asc().nullsLast().op("bool_ops")).where(sql`(is_public = true)`),
